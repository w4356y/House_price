...
output:
  xaringan::moon_reader:
    css: [default, zh-CN.css, test.css]
    lib_dir: libs
    nature:
...
.table[
.conclusion[
```{r kabel, echo=FALSE,warning=FALSE}
library(kableExtra)
knitr::kable(head(mtcars), format = "html", align = 'c', booktabs =T) %>% kable_styling(full_width = F) %>% column_spec(c(2:7), width = "2cm") %>% column_spec(1, width = "8cm", color = "red", bold = T) %>% row_spec(2, strikeout = T) %>% row_spec(3:5, bold = T, color = "white", background = "black")
```
]]
---
class: center, middle
...
library(dplyr)
mtcars[1:10, 1:2] %>% mutate(car = row.names(.),
                             mpg = cell_spec(mpg, "html", color = ifelse(mpg > 20, "red", "blue")), 
                             cyl = cell_spec(cyl, "html", color = "white", align = "c",
                                             angle = 45, background = factor(cyl, c(4, 6, 8), c("#666666", "#999999", "#BBBBBB")
                                                                             )
                                             )) %>%
  select(car, mpg, cyl) %>%
  kable("html", escape = F, booktabs = T, linesep = "")
```

---
class: center, middle

```{r specification_color, echo = FALSE, warning=FALSE, message=FALSE}
iris[1:10, ] %>%
...
---
class: center, middle
```{r test1, echo=FALSE, warning=FALSE, message=FALSE}
dt = head(mtcars)
kable(head(mtcars), "html", booktabs = T) %>%
  kable_styling(full_width = T) %>%
  add_header_above(c(" " = 1, "Group 1" = 3, "Group 2" = 4, "Group 3" = 4))
```
---
class: center, middle
```{r test2, echo=FALSE, warning=FALSE, message=FALSE}
kable(dt, "html", booktabs = T) %>% 
  kable_styling(full_width = T) %>%
  add_header_above(c(" ", "Group 1" = 3, "Group 2" = 4, "Group 3" = 4)) %>%
  add_header_above(c(" ", "Group 4" = 6, "Group 5" = 5)) %>%
  add_header_above(c(" ", "Group 6" = 11), bold = T, italic = T)
```
---
class: center, middle
```{r test3, echo=FALSE, warning=FALSE, message=FALSE}
kable(mtcars[1:10, 1:6], "html", caption = "Group Rows", booktabs = T) %>% 
  kable_styling() %>%
...
---
class: center, middle
```{r test4, echo=FALSE, warning=FALSE, message=FALSE}
collapse_rows_dt <- data.frame(C1 = c(rep("a", 10), rep("b", 5)),
                               C2 = c(rep("c", 7), rep("d", 3), rep("c", 2), rep("d", 3)),
                               C3 = 1:15,
                               C4 = sample(c(0,1), 15, replace = TRUE))
kable(collapse_rows_dt[-1], "html", align = "c", booktabs = T) %>%
...
---
class: center, middle
```{r test5, echo=FALSE, warning=FALSE, message=FALSE}
